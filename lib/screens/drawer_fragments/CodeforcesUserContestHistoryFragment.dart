...
class CodeforcesUserContestHistoryFragment extends StatelessWidget {
  List<RatingChange> ratingChanges;
  double widthPiece, heightPiece;
  SizeUtil size;

  CodeforcesUserContestHistoryFragment({Key key, @required this.ratingChanges})
      : super(key: key);
...
class RatingChangeTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onTap: () async {
        bool isSuccessful =
            await launch(CodeforcesAPIService.getContestURL(ratingChange.cid));
